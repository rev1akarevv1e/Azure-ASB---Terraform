locals {
  ...
  #This block allows for multiple queues within each ASBN
...
#This allows for granular control of resource lock. 
# var.enable is used to lock all resources created using this module
# var.resource_lock_enabled can be set to false in order to disable resource locking on specific ASBN
resource "azurerm_management_lock" "service-bus-lock" {
  count      = var.enabled && var.resource_lock_enabled ? 1 : 0
  name       = format("%s-service-bus-lock", var.lock_level)
  scope      = azurerm_servicebus_namespace.main.id
  lock_level = var.lock_level
  notes      = "This Service Bus Namespace is locked by Terraform"
}
